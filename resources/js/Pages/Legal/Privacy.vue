...
onMounted(() => {
    if (props.termlyCode) {
        ...
        div.innerHTML = props.termlyCode;
        ...
        <div class="fixed top-0 left-0 right-0 z-50 py-4 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto flex justify-between items-center">
                ...
                <nav class="flex gap-4 sm:gap-6 items-center">
                    <Link :href="route('about')" class="text-white font-bold text-sm sm:text-base hover:text-yellow-300 transition-colors">About</Link>
                    <Link :href="route('terms')" class="text-white font-bold text-sm sm:text-base hover:text-yellow-300 transition-colors">Terms</Link>
                    <Link :href="route('privacy')" class="text-yellow-300 font-bold text-sm sm:text-base">Privacy</Link>
        ...
        <div class="pt-32 pb-12 relative z-10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                    ...
                    <div class="space-y-6">
                        <CartoonCard>
                            <div v-if="termlyCode" v-html="termlyCode" class="prose prose-lg max-w-none"></div>
                            <div v-else class="prose prose-lg max-w-none">
                                <h1>Privacy Policy</h1>
                                <p>Please add your Termly.io embed code in the admin panel.</p>
                            </div>
                        </CartoonCard>
                    </div>
                </div>
